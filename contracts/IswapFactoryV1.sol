...
contract IswapV1Pair is IIswapV1Pair, IswapV1ERC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (amount0Out > 0) { 
            _safeTransfer(_token0, to, amount0Out.sub(amount0Out / uint(2000)));
            _safeTransfer(_token0, IIswapV1Factory(factory).feeTo(), amount0Out / uint(2000));    //  0.05% / 100% = 5 / 10000 = 1 / 2000
        } // optimistically transfer tokens
        if (amount1Out > 0) {
            _safeTransfer(_token1, to, amount1Out.sub(amount1Out / uint(2000)));
            _safeTransfer(_token1, IIswapV1Factory(factory).feeTo(), amount1Out / uint(2000));    //  0.05% / 100% = 5 / 10000 = 1 / 2000
        } // optimistically transfer tokens
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'IswapV1: K');
        }
